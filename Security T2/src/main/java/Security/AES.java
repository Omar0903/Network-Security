package Security;

import java.util.Arrays;

public class AES {

    private static final int Nb = 4;
    private static final int Nk = 4;
    private static final int Nr = 10;

    private static final int[] SBOX = {
        
    };

    private static final int[] INV_SBOX = new int[256];
    static {
        for (int i = 0; i < 256; i++) INV_SBOX[SBOX[i]] = i;
    }

    private static final int[] RCON = {0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36};

    public String encrypt(String plainText, String key) {
        int[] state = hexToBytes(plainText);
        int[][] roundKeys = keyExpansion(hexToBytes(key));

        addRoundKey(state, roundKeys[0]);

        for (int r = 1; r < Nr; r++) {
            subBytes(state, SBOX);
            shiftRows(state);
            mixColumns(state);
            addRoundKey(state, roundKeys[r]);
        }

        subBytes(state, SBOX);
        shiftRows(state);
        addRoundKey(state, roundKeys[Nr]);

        return "0x" + toHex(state);
    }

    public String decrypt(String cipherText, String key) {
        int[] state = hexToBytes(cipherText);
        int[][] roundKeys = keyExpansion(hexToBytes(key));

        addRoundKey(state, roundKeys[Nr]);

        for (int r = Nr - 1; r > 0; r--) {
            invShiftRows(state);
            subBytes(state, INV_SBOX);
            addRoundKey(state, roundKeys[r]);
            invMixColumns(state);
        }

        invShiftRows(state);
        subBytes(state, INV_SBOX);
        addRoundKey(state, roundKeys[0]);

        return "0x" + toHex(state);
    }

    private void subBytes(int[] s, int[] box) {
        for (int i = 0; i < 16; i++) s[i] = box[s[i]];
    }

    private void shiftRows(int[] s) {
        int[] t = Arrays.copyOf(s, 16);
        s[1]=t[5]; s[5]=t[9]; s[9]=t[13]; s[13]=t[1];
        s[2]=t[10]; s[6]=t[14]; s[10]=t[2]; s[14]=t[6];
        s[3]=t[15]; s[7]=t[3]; s[11]=t[7]; s[15]=t[11];
    }

    private void invShiftRows(int[] s) {
        int[] t = Arrays.copyOf(s, 16);
        s[1]=t[13]; s[5]=t[1]; s[9]=t[5]; s[13]=t[9];
        s[2]=t[10]; s[6]=t[14]; s[10]=t[2]; s[14]=t[6];
        s[3]=t[7]; s[7]=t[11]; s[11]=t[15]; s[15]=t[3];
    }

    private void mixColumns(int[] s) {
        for (int c = 0; c < 4; c++) {
            int i = 4*c;
            int a0=s[i], a1=s[i+1], a2=s[i+2], a3=s[i+3];
            s[i]=mul(2,a0)^mul(3,a1)^a2^a3;
            s[i+1]=a0^mul(2,a1)^mul(3,a2)^a3;
            s[i+2]=a0^a1^mul(2,a2)^mul(3,a3);
            s[i+3]=mul(3,a0)^a1^a2^mul(2,a3);
        }
    }

    private void invMixColumns(int[] s) {
        for (int c = 0; c < 4; c++) {
            int i = 4*c;
            int a0=s[i], a1=s[i+1], a2=s[i+2], a3=s[i+3];
            s[i]=mul(14,a0)^mul(11,a1)^mul(13,a2)^mul(9,a3);
            s[i+1]=mul(9,a0)^mul(14,a1)^mul(11,a2)^mul(13,a3);
            s[i+2]=mul(13,a0)^mul(9,a1)^mul(14,a2)^mul(11,a3);
            s[i+3]=mul(11,a0)^mul(13,a1)^mul(9,a2)^mul(14,a3);
        }
    }

    private void addRoundKey(int[] s, int[] k) {
        for (int i = 0; i < 16; i++) s[i] ^= k[i];
    }

    private int mul(int a, int b) {
        int res=0;
        while(b>0){
            if((b&1)!=0) res^=a;
            a=(a<<1)^((a&0x80)!=0?0x11B:0);
            b>>=1;
        }
        return res&0xFF;
    }

    private int[][] keyExpansion(int[] key) {
        int[][] w = new int[44][4];
        for(int i=0;i<4;i++)
            for(int j=0;j<4;j++) w[i][j]=key[4*i+j];

        for(int i=4;i<44;i++){
            int[] temp = Arrays.copyOf(w[i-1],4);
            if(i%4==0){
                int t=temp[0]; temp[0]=temp[1]; temp[1]=temp[2]; temp[2]=temp[3]; temp[3]=t;
                for(int j=0;j<4;j++) temp[j]=SBOX[temp[j]];
                temp[0]^=RCON[i/4];
            }
            for(int j=0;j<4;j++) w[i][j]=w[i-4][j]^temp[j];
        }

        int[][] roundKeys=new int[11][16];
        for(int r=0;r<11;r++)
            for(int c=0;c<4;c++)
                System.arraycopy(w[r*4+c],0,roundKeys[r],4*c,4);

        return roundKeys;
    }

    private int[] hexToBytes(String hex){
        if(hex.startsWith("0x")) hex=hex.substring(2);
        int[] out=new int[hex.length()/2];
        for(int i=0;i<hex.length();i+=2)
            out[i/2]=Integer.parseInt(hex.substring(i,i+2),16);
        return out;
    }

    private String toHex(int[] arr){
        StringBuilder sb=new StringBuilder();
        for(int i:arr) sb.append(String.format("%02X",i));
        return sb.toString();
    }
}
